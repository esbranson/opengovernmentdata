...
##
# qcew2rdf - convert the US BLS quarterly Census of Employment and Wages dataset into RDF
#
...
Usage:  qcew2rdf [options] GOVT_UNITS_*.txt *.q1-q4.singlefile.csv [output]
Arguments:
...
#		qtrly_contributions = row[14] # XXX ever non-zero?
		...
		for lvl,month in {(month1_emplvl,month1_date), (month2_emplvl,month2_date), (month3_emplvl,month3_date)}:
			url = bls_cew['_'.join(['emplvl',aid,industry_code,owner_code,month])]
			...
			g.add((url, freqprop, freqvalm))
			g.add((url, tpprop, rdflib.Literal(month, datatype=dtgym)))
			g.add((url, peopvalprop, rdflib.Literal(lvl, datatype=dtnni)))

		url = bls_cew['_'.join(['avgwwage',aid,industry_code,owner_code,qdate])]
		g.add((url, rdflib.RDF.type, obstype))
		g.add((url, rdflib.RDF.type, avgwwagetype))
		g.add((url, areaprop, area))
		g.add((url, indprop, naics_ind[industry_code]))
		g.add((url, ownprop, naics_own[owner_code]))
		g.add((url, freqprop, freqvalq))
		g.add((url, tpprop, rdflib.Literal(qdate, datatype=dtgym)))
		g.add((url, curvalprop, rdflib.Literal(avg_wkly_wage, datatype=dti)))
